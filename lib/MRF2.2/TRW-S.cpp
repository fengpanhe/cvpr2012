...
#define TRUNCATE_MIN(a, b) \
    {                      \
        if ((a) > (b))     \
            (a) = (b);     \
    }
#define TRUNCATE_MAX(a, b) \
    {                      \
        if ((a) < (b))     \
            (a) = (b);     \
    }
...
TRWS::TRWS(int width, int height, int nLabels, EnergyFunction* eng)
    : MRF(width, height, nLabels, eng)
{
    Allocate();
}
TRWS::TRWS(int nPixels, int nLabels, EnergyFunction* eng)
...
TRWS::~TRWS()
{
    ...
    if (m_DBinary)
        delete[] m_DBinary;
    if (m_horzWeightsBinary)
        delete[] m_horzWeightsBinary;
    if (m_vertWeightsBinary)
        delete[] m_vertWeightsBinary;
}
...
void TRWS::setSmoothness(SmoothCostGeneralFn cost)
{
    ...
    if (!m_allocateArrayForSmoothnessCostFn)
        return;
    ...
    if (m_type != BINARY) {
        ...
        for (y = 0; y < m_height; y++)
            for (x = 0; x < m_width; x++, D_ptr += K, M_ptr += 2 * K, n++) {
                ...
                if (x < m_width - 1)
                    AddVector(Di, M_ptr, K); // message (x+1,y)->(x,y)
                if (y < m_height - 1)
                    AddVector(Di, M_ptr + K, K); // message (x,y+1)->(x,y)
    ...
    } else // m_type == BINARY
    {
        ...
        for (y = 0; y < m_height; y++)
            for (x = 0; x < m_width; x++, M_ptr += 2, n++) {
                Di = m_DBinary[n];
                if (x > 0)
                    Di += (m_answer[n - 1] == 0) ? m_horzWeightsBinary[n - 1] : -m_horzWeightsBinary[n - 1];
                if (y > 0)
                    Di += (m_answer[n - m_width] == 0) ? m_vertWeightsBinary[n - m_width] : -m_vertWeightsBinary[n - m_width];

                if (x < m_width - 1)
                    Di += M_ptr[0]; // message (x+1,y)->(x,y)
                if (y < m_height - 1)
                    Di += M_ptr[1]; // message (x,y+1)->(x,y)
    ...
    for (; nIterations > 0; nIterations--) {
        ...
        for (y = 0; y < m_height; y++)
            for (x = 0; x < m_width; x++, D_ptr += K, M_ptr += 2 * K, n++) {
                CopyVector(Di, D_ptr, K);
                if (x > 0)
                    AddVector(Di, M_ptr - 2 * K, K); // message (x-1,y)->(x,y)
                if (y > 0)
                    AddVector(Di, M_ptr - (2 * m_width - 1) * K, K); // message (x,y-1)->(x,y)
                if (x < m_width - 1)
                    AddVector(Di, M_ptr, K); // message (x+1,y)->(x,y)
                if (y < m_height - 1)
                    AddVector(Di, M_ptr + K, K); // message (x,y+1)->(x,y)
    ...
    for (; nIterations > 0; nIterations--) {
        ...
        for (y = 0; y < m_height; y++)
            for (x = 0; x < m_width; x++, M_ptr += 2, n++) {
                Di = m_DBinary[n];
                if (x > 0)
                    Di += M_ptr[-2]; // message (x-1,y)->(x,y)
                if (y > 0)
                    Di += M_ptr[-2 * m_width + 1]; // message (x,y-1)->(x,y)
                if (x < m_width - 1)
                    Di += M_ptr[0]; // message (x+1,y)->(x,y)
                if (y < m_height - 1)
                    Di += M_ptr[1]; // message (x,y+1)->(x,y)
        ...
        for (y = m_height - 1; y >= 0; y--)
            for (x = m_width - 1; x >= 0; x--, M_ptr -= 2, n--) {
                ...
                if (y > 0) {
                    ...
                    if (Di > lambda)
                        M_ptr[-2 * m_width + 1] = lambda;
                    else
                        M_ptr[-2 * m_width + 1] = (Di < -lambda) ? -lambda : Di;
                }
            }
    }
            ...
            for (x = 0; x < m_width; x++, D_ptr += K, M_ptr += 2 * K, V_ptr += 2 * K * K, n++) {
                ...
                if (x < m_width - 1) {
                    if (m_V)
                        ...
                        UpdateMessageGENERAL(M_ptr, Di, K, 0.5, m_smoothFn, n, n + 1, buf);
                }
                if (y < m_height - 1) {
                    if (m_V)
                        UpdateMessageGENERAL(M_ptr + K, Di, K, 0.5, /* forward dir*/ 0, V_ptr + K * K, buf);
                    else
                        UpdateMessageGENERAL(M_ptr + K, Di, K, 0.5, m_smoothFn, n, n + m_width, buf);
                }
            }
            ...
            for (x = m_width - 1; x >= 0; x--, D_ptr -= K, M_ptr -= 2 * K, V_ptr -= 2 * K * K, n--) {
                ...
                if (x > 0) {
                    if (m_V)
                        ...
                        m_lowerBound += UpdateMessageGENERAL(M_ptr - 2 * K, Di, K, 0.5, m_smoothFn, n, n - 1, buf);
                }
                if (y > 0) {
                    if (m_V)
                        m_lowerBound += UpdateMessageGENERAL(M_ptr - (2 * m_width - 1) * K, Di, K, 0.5, /* backward dir */ 1, V_ptr - (2 * m_width - 1) * K * K, buf);
                    else
                        m_lowerBound += UpdateMessageGENERAL(M_ptr - (2 * m_width - 1) * K, Di, K, 0.5, m_smoothFn, n, n - m_width, buf);
                }
            }
    }
