...
// uncomment "#define COUNT_TRUNCATIONS" in energy.h to enable counting of
// truncations
...
EnergyFunction* generate_DataARRAY_SmoothFIXED_FUNCTION()
{
    ...
    MRF::CostVal* ptr;
    for (ptr = &D[0]; ptr < &D[sizeX * sizeY * numLabels]; ptr++)
        *ptr = ((MRF::CostVal)(rand() % 100)) / 10 + 1;
...
{
    if (pix2 < pix1) { // ensure that fnCost(pix1, pix2, i, j) == fnCost(pix2,
        // pix1, j, i)
        int tmp;
        tmp = pix1;
        pix1 = pix2;
...
#ifdef COUNT_TRUNCATIONS
            if (truncCnt > 0)
                printf("***WARNING: %d terms (%.2f%%) were truncated to ensure "
                       "regularity\n",
