...
ejunctions = cat(1, e2j{:});%e2j = neighbors.fragment_junctionlist;list中得到，按列拼接
...
jadj = cell(nj, 1);
for k = 1:ne   
    jadj{ejunctions(k, 1)}(end+1, 1) = k;  % forward  ejunctions存的是第i条边的两个端点
...
% eadj存储的是边的临近边id，jadj存的是每个点相交的边。eadj通过两端结点来找jadj，从而得到临近边。
