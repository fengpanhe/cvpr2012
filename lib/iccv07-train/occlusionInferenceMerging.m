function [lab, energy, edgeletChain, chainLR, regions] = ...
    occlusionInferenceMerging(pB, pC, edgeind, bndinfo, delta)
%
...
for k = 1:numel(eprev)    
    if ~isempty(eprev{k})
        %Eij{k} = -log(full(pCm(k, eprev{k})));
        %Eon(k) = min(Eon(k), min(Eij{k}));
        Eon(k) = min(Eij{k});
    end
end
Eoff = -log(pB(:, 1))+delta; % energy for edge being off
...
displayRegions(regions, bndinfo.wseg, 1);




%% display function
function displayRegions(rsp, wseg, fignum)
