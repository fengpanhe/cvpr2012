function [lab, result, valdata, energy, dispim] = ...
    mergeMin(pB, bndinfo, delta, minRegions, DO_VAL)
%
...
for k = 1:numel(echain)
    ...
    Ecost(k) = max(log((1-pB(echain{k},1)) ./ pB(echain{k},1)));
end

nregions = numel(validr);

iter = 0;
while (mincost < delta) && nregions > minRegions
...
%                 Ecost(c) = max(log((1-pB(echain{c},1)) ./ pB(echain{c},1)));
%             end
%         end
%     end    
...
%     disp(num2str(['iter: ' num2str([iter  mincost])]));
%     disp(num2str(rsp{r1}))
%     disp(num2str(rsp{r2}))  
...
%                Ecost(i1) = sum(Eoff(echain{i1}))-sum(Eon(echain{i1}));
                Ecost(i1) = max(log((1-pB(echain{i1},1)) ./ pB(echain{i1},1)));
...
for c = 1:npair
    ...
    lab(mod(echain{c}-1, ne)+1) = minlab;
end 
