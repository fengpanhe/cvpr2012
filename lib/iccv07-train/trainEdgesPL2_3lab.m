function alpha = trainEdgesPL2_3lab(bndinfo, pB, pC, edgeind, Y)
...
for f = 1:numel(bndinfo)
    ...
    % get unary term                                          
    pB{f} = [pB{f}(1:ne, 1:2) pB{f}(ne+1:nde, 2)];
    x1{f} = pB{f};             
    x1{f}(:, 2:3) = 1;
    x3{f} = [ones([ne 1]) pB{f}(:, 2:3)];
    ...
    for k = 1:size(edgeind{f}, 1)
        ...
        if Y{f}(k1) > 0 && lab1==lab2
            x2{f}(ku2, 2+dct) = pC{f}(k); % / tmppB(k2, 2); % / dtBnd.prior(1+(k2>ne));
            %isset2(k2) = true;
        end        
    end  
%     x2{f} = [1-x2{f} x2{f}];
...
%     prior{f}(ne+1:nde, 2) = dtBnd.prior(3);
    ...
    ty{f} = (Y{f}(1:ne)>0) + 2*(Y{f}(ne+1:2*ne)>0);
...
x{1} = [ones(ndata, 1) zeros(ndata, 1) zeros(ndata, 1)]; %xp; %repmat(dtBnd.prior, [ndata 1]);
x{2} = log(x1);
x{3} = log(x2);
x{4} = log(x3);
