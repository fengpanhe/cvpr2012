function [lab, result, energy, dispim] = ...
    mergeEnergyBased(pB, pC, edgeind, bndinfo, offBias)
%
% [lab, energy, result, valdata, dispim] = ...
%   mergeEnergyBased(pB, pC, edgeind, bndinfo, thresh, DO_VAL))
%
...
if exist('valthresh', 'var') && ~isempty(valthresh)
    DO_VAL = 1;    
    currv = 1; 
    nval = numel(valthresh);
    stats = regionprops(bndinfo.wseg, 'Area');
    areas = cat(1, stats(:).Area);
    segError = zeros(nval, 1);
    mergeCost = zeros(nval, 1);    
    valRegions = zeros(nval, 1);
else
    DO_VAL = 0;
    valdata = [];
end
...
enext = cell(nde, 1);
eprev = cell(nde, 1);
Eij = cell(nde, 1); % pairwise energy lists
for k = 1:size(edgeind, 1)
    k1 = edgeind(k, 1);
    k2 = edgeind(k, 2);    
    enext{k1}(end+1) = k2;
    eprev{k2}(end+1) = k1;
    Eij{k2}(end+1) = -log(pC(k));
end
...
unary = -log(pB(:, 2));
Eon = unary; % lower bound for energy of edge being on
for k = 1:numel(eprev)    
    if ~isempty(eprev{k})
        Eon(k) = min(Eij{k});
    end
end
Eoff = -log(pB(:, 1)) + offBias; % energy for edge being off
...
nregions = numel(validr);
iter = 0;
while (mincost < 0) && nregions>1
    ...
    if DO_VAL && (currv <= nval) && mincost > (valthresh(currv))       
        nsp = max(wseg(:));
        segError(currv) = ...
            getSegmentationErrorFast(bndinfo.labels, nsp, rsp(validr), areas);
        mergeCost(currv) = valthresh(currv);
        valRegions(currv) = sum(validr);
        currv = currv + 1;
    end  
...
if DO_VAL
    valdata.mergeCost = mergeCost(1:currv-1);
    valdata.segError = segError(1:currv-1);
    valdata.nregions = valRegions(1:currv-1);
end
