function [tx, cf] = getContinuityFeatures2(X, bndinfo, c1, nextc, pB, bndx)

nextc = reshape(nextc, [1 numel(nextc)]);
nc2 = numel(nextc);

tx = zeros(nc2, 12); % features: continuity + pContour of same type
...
%                  X(c2, [1:15 17:end]) ]; 
    ...
    if c2 <= ne
        ...
        tx(tc, 1:8) = [relAngle(tc)       abs(relAngle(tc)) ...
                        pB(c1, 1:2)        pB(c2, 1:2) ...
