function [chains, e2chain, chainsize] = chainEdgelets(pbVal, enext, thetaStart, ...
    thetaEnd, pbThresh, thetaThresh, doskip, jpos1, jpos2, pixrange, elen)
...
for k = 1:numel(enext)
    ...
    if elen(k)>4
        ind = (ra{k} <= thetaThresh) | (elen(enext{k})<=4);    
        ra{k} = ra{k}(ind);
    ...
    if doskip
        ...
        for k2 = others
            if ~isempty(enext{k2})
                ra2 = mod(abs(thetaEnd(k)-thetaStart(enext{k2})), 180+1E-3);
                jp1 = jpos2(k*ones(numel(enext{k2}),1), :)';
                jp2 = jpos1(enext{k2}, :)';
                ind = (ra2 <= thetaThresh) & (sum((jp1-jp2).^2,1)<=pixrange);
                k3 = enext{k2}(ind);
                enext2{k} = [enext2{k} k3];
                for k4 = k3
                    eprev{k4}(end+1) = k;
                end
                ra{k} = [ra{k} ra2(ind)];
