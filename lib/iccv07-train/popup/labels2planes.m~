function [gplanes, vplanes, gim, vim] = ...
    labels2planes(bndinfo, labels, v0, yc, contactDt, im)
...
% get pixel map of regions
stats = regionprops(bndinfo.wseg, 'PixelIdxList');
pixidx = {stats(:).PixelIdxList};
rlab = zeros(bndinfo.nsp, 1);
for k = 1:numel(regions)
    rlab(regions{k}) = k;
end  
rmap = rlab(bndinfo.wseg);
...
for k = 1:numel(vind)       
    ...
    [cu, cv] = poly2contacts2(contactDt, jpos(:,1), jpos(:,2), ...
        bndinfo.imsize, v0, yc, []);
    cu = cu(rgeom(jregion)==1);
    cv = cv(rgeom(jregion)==1);
    
    % XXX WORK FROM HERE
    
    %% Warp vertical objects to frontal view
    % Save separate texture maps for each vertical object.
    if isempty(cu) % no contact points found
    ...
    if numel(cu==1)
        ...
        vplanes(k).gfit =  ...
            [cu(:).*(imw-1)./(imh-1)  (cv-1)/(imh-1) 
        vplanes(k).sinfo{c}.x = 
        for c = 1:numel(cu)
            vplanes(k).sinfo{c}.x = cu(c);
            vplanes(k).sinfo{c}.y = cv(c);
        end
