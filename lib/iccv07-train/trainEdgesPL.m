...
for f = 1:numel(bndinfo)
    ...
    for k = 1:size(edgeind, 1)
        ...
        k2 = edgeind(k, 2);    
        if doAll
            x2{f}(k2) = max(x2{f}(k2), pC(k)*tmppB(k1, 2)); % / tmppB(k2, 2); %dtBnd.prior(1+(k2>ne));
            isset2(k2) = true;
        else            
            lab1 = bndinfo(f).labels(spLR(k1, 1));
            lab2 = bndinfo(f).labels(spLR(k2, 1));            
            if Y{f}(k1) > 0 && lab1==lab2
                x2{f}(k2) = pC(k)*tmppB(k1, 2);% / tmppB(k2, 2); % / dtBnd.prior(1+(k2>ne));
                isset2(k2) = true;
            end
        end
    end  
...
x{1} = xp; %repmat(dtBnd.prior, [ndata 1]);
x{2} = log(x1);
x{3} = log(x2);
%x{4} = log(x3);
...
%     ndata = numel(ty);
%     x{1} = [repmat(dtBnd.prior(1), [ndata 1]) ...
%         [repmat(dtBnd.prior(2), [ndata/2, 1]) ; repmat(dtBnd.prior(3), [ndata/2, 1])]];
