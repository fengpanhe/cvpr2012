function [classifier, xall, yall] = ...
    trainFastBoundaryClassifier(X, Y, bndinfo, do_weights)
% trains a 2-class boundary on classifier

w = [];
[tx, ty, categoryFeatures, xall, yall] = reformatData(X, Y, bndinfo);                


if exist('do_weights', 'var') && do_weights
    w = tx(:, 10); % weight by area of smaller region
    w = w ./ sum(w);
    disp(num2str(sum(-w.*log(w))/log(numel(w))))
else
    disp('no data weighting');
end
...
nnodes = 16;
ntrees = 10;
classifier = train_boosted_dt_2c(tx, categoryFeatures, (ty>0)*2-1, ntrees, nnodes, 0, w);
...
for f = 1:numel(X)    
    ...
    ind = [ind ; ind];
    Y{f}(ind) = [];  
    [tx{f}, cf] = getFastBoundaryClassifierFeatures(bndinfo(f), X(f), xind); % was Features4
